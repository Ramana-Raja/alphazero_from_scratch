...
import numpy as np
import math
import pytorch.alphazero.model_components.encoder_decoder as ed
from pytorch.alphazero.model_components.chess_board import board as c_board
import copy
import torch
import datetime
...
class UCTNode():
    ...
    Parameters
    ----------
    game: chess_board objet
        holds current state of a board
    move: move that created this position from parent
    parent : parent UCTNode
    ...
    def child_Q(self):
        return self.child_total_value / (1 + self.child_number_visits) # 1 to prevent 0 division
    ...
    def maybe_add_child(self, move):
        """
        make the move on the board and saves it as a child of current positon
        """
...
def UCT_search(game_state, num_reads, net, batch_size=64):
    ...
    while simulations_done < num_reads:
        leaf_nodes = []
        encoded_states = []
        ...
        input_batch = torch.stack(encoded_states).cuda()  # (B, 18, 8, 8)
        with torch.no_grad():
            child_priors_batch, value_estimates = net(input_batch)
        child_priors_batch = child_priors_batch.detach().cpu().numpy()  # (B, 4672)
        value_estimates = value_estimates.detach().cpu().numpy().reshape(-1)  # (B,)

        # Expand and backup for each leaf
        for i, leaf in enumerate(leaf_nodes):
            child_priors = child_priors_batch[i]
...
def save_as_pickle(filename, data,index):
    dir_path = f"C:\\Users\\raman\\PycharmProjects\\pythonProject1\\pytorch\\alphazero\\model_components\\datasets\iter{index}"
    completeName = os.path.join(dir_path, filename)
...
def MCTS_self_play(chessnet, num_games, cpu,index):
    ...
    for idxx in range(0, num_games):
        ...
        del dataset
        save_as_pickle("dataset_cpu%i_%i_%s" % (cpu, idxx, datetime.datetime.today().strftime("%Y-%m-%d")), dataset_p,index = index)
